...
pub fn lookup_packet(state: ProtocolState, packet: u8) -> Option<PacketHandler> {
    match state {
        ...
        // IDs re-derived from Minestom 2026.07.12-26.2's SERVER_PLAY registry order
        // (net.minestom.server.network.packet.PacketVanilla, protocol 776) — the
        // upstream IDs here were for an earlier protocol (1.21.8/772) and every one
        // of them had drifted. This was the actual cause of the "bots join then go
        // silently unresponsive with no disconnect message" symptom found while
        // building the P0 bench harness: keep_alive at the wrong ID meant the bot
        // never answered the server's keep-alive, and disconnect/kick was ALSO at
        // the wrong ID, so the resulting timeout-kick was never recognized either
        // — see VENDOR.md.
        ProtocolState::Play => match packet {
            ...
            0x31 => return Some(play::process_join_game),
            0x20 => return Some(play::process_kick),
            0x48 => return Some(play::process_teleport),
            0x81 => return Some(config::process_transfer),
            _ => {}
        },
    }
    None

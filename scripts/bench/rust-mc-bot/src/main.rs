...
const PROTOCOL_VERSION: u32 = 776; // minecom targets MC 26.2 (Minestom MinecraftConstants.PROTOCOL_VERSION)
...
fn main() -> io::Result<()> {
    ...
    if args.len() < 3 {
        ...
        #[cfg(unix)]
        println!("usage: {} <ip:port or path> <count> [threads] [name_offset]", name);
        #[cfg(not(unix))]
        println!("usage: {} <ip:port> <count> [threads] [name_offset]", name);
    ...
    let arg3 = args.get(3);
    // name_offset (arg4): lets a caller run multiple invocations against the same
    // server without Bot_0.. username/offline-UUID collisions kicking the earlier
    // batch — vendored addition for the P0 benchmark harness's ramp mode
    // (scripts/bench/run_scenario.py launches successive batches to find max bots
    // before sub-20 TPS). Not in upstream; see VENDOR.md.
    let arg4 = args.get(4);
    ...
    let mut names_used: u32 = match arg4 {
        Some(str) => str
            .parse()
            .unwrap_or_else(|_| panic!("{} is not a number", str)),
        None => 0,
    };
